...
public class UserController extends AbstractAuthentication {
    ...
    public Result<String> user() {
        return new Result<>(getAuthentication().username() + " " + getAuthentication().uid());
    }

    @PostMapping("/register")
    public Result<RegisterResponse> register(@RequestBody RegisterRequest request) {
        RegisterInfo register = new RegisterInfo(request.username(), request.password(), generatorHelper.generateUUID());
